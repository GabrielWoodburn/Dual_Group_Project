...
public class StartProgram {
		...
		private static void editAnItem() {
			...
			if (searchBy == 1) {
				...
				foundItems = lih.searchForItemByZoo(zooName);
			} else {
				System.out.print("Enter the item: ");
				String animalName = in.nextLine();
				foundItems = lih.searchForItemByAnimal(animalName);
			}
			...
			if (!foundItems.isEmpty()) {
				...
				System.out.println("Retrieved " + toEdit.getAnimal() + " from " + toEdit.getZoo());
				...
				if (update == 1) {
					...
					String newZoo = in.nextLine();
					toEdit.setZoo(newZoo);
				} else if (update == 2) {
